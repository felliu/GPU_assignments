...
#define NUM_PARTICLES_DEFAULT 1000000
#define NUM_ITERATIONS_DEFAULT 10000
...
void particleGPU(int num_particles, int num_iters, float3 *Particle, float3 *Velocity, float updateValue, int tpb)
{
    ...
    cudaMalloc(&d_particle, num_particles * sizeof(float3));
    cudaMalloc(&d_velocity, num_particles * sizeof(float3));

    cudaMemcpy(d_particle, Particle, num_particles * sizeof(float3), cudaMemcpyHostToDevice);
    cudaMemcpy(d_velocity, Velocity, num_particles * sizeof(float3), cudaMemcpyHostToDevice);

    for (int i = 0; i < num_iters; ++i)
        particleKernel<<<(num_particles + tpb - 1) / tpb, tpb>>>(num_particles, d_particle, d_velocity, i, updateValue);
    cudaMemcpy(Particle, d_particle, num_particles * sizeof(float3), cudaMemcpyDeviceToHost);
}
...
int main(int argc, char *argv[])
{
    ...
    float3 *Particle_GPU = (float3*) malloc(num_particles * sizeof(float3));
    float3 *Velocity = (float3*) malloc(num_particles * sizeof(float3));
    ...
    memcpy(Particle_GPU, Particle_CPU, num_particles * sizeof(float3));
