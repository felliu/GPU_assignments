...
"   int item_z = get_local_id(2);             \n"
"   printf(\"Hello World! group: (%d, %d, %d), item: (%d, %d, %d)\\n\",\n"
"            group_x, group_y, group_z, item_x, item_y, item_z);}\n;";
...
int main(int argc, char *argv[]) {
  ...
  cl_program program = clCreateProgramWithSource(context, 1, &mykernel, NULL, &err);CHK_ERROR(err);
  ...
  const size_t g_worksz_3d[] = {16, 4, 4};
  const size_t l_worksz_3d[] = {16, 4, 4};
  err = clEnqueueNDRangeKernel(cmd_queue, kernel, ndims, NULL,
                               g_worksz_3d, l_worksz_3d,
                               0, NULL, NULL);CHK_ERROR(err);
